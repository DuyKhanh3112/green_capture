...
import 'package:green_capture/api/api_service.dart';
  ...
  void detectPicture() async {
    try {
      ...
      for (var item in jsonResponse['results']) {
        Get.find<MainController>().listImageDetect.add(ImageDetect.fromJson({
              'image': item['image'],
              ...
              CustomWidget.homeBtn(
                  onTap: Get.find<MainController>().detecting.value == true
                      ...
                      : () async {
                          ...
                          if (image != null) {
                            Get.find<MainController>().clickDetect.value =
                                false;
